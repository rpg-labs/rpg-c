...
int rpg_list_init( apr_pool_t *p, struct _rpg_list **out_new ) {
	struct _rpg_list *new = ( struct _rpg_list * ) apr_palloc ( p, sizeof(struct _rpg_list) );FAIL_IF_NULL(new);
	new->first = NULL;
	new->last = NULL;
	new->length=0;
...
int rpg_new_list_item( apr_pool_t *p, void *data, struct _rpg_list_item **out_new ) {
	...
	struct _rpg_list_item *new = ( struct _rpg_list_item * ) apr_palloc ( p, sizeof(struct _rpg_list_item) );FAIL_IF_NULL(new)
	new->data = data;
	new->next = NULL;
...
int rpg_list_add(apr_pool_t *p, struct _rpg_list *list, void *data) {
	...
	int ret = rpg_new_list_item( p, data, &new );ENSURE_SUCCEEDED
